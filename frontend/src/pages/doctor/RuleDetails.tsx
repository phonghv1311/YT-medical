...
function normalizeRuleDetail(r: unknown): RuleData {
  const d = r as Record<string, unknown>;
  const rawDate = (d?.updatedAt ?? d?.modifiedAt ?? d?.lastUpdated) as string | undefined;
  return {
    name: (d?.name as string) ?? (d?.title as string) ?? 'Untitled',
    lastUpdated: rawDate ? new Date(rawDate).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : '—',
    category: (d?.category as string) ?? '—',
    trigger: (d?.trigger as string) ?? '—',
    status: (d?.status as string) === 'INACTIVE' ? 'INACTIVE' : 'ACTIVE',
    conditions: Array.isArray(d?.conditions) ? (d.conditions as { field?: string; op?: string; value?: number }[]).map((c) => ({
...
export default function DoctorRuleDetails() {
  ...
  const [rule, setRule] = useState<RuleData | null | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    doctorsApi.me.getRuleById(id)
      .then((res) => {
        if (cancelled) return;
        setRule(normalizeRuleDetail(res.data));
      })
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 pb-24">
        <div className="pt-2 mb-4">
          <Link to="/doctor/rules" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">Back</Link>
  ...
  if (rule === null) {
    return (
      <div className="max-w-2xl mx-auto px-4 pb-24">
        ...
        <div className="py-12 text-center text-gray-500">Rule not found.</div>
      </div>
    );
  }
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-5 mb-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Last updated: {rule.lastUpdated}</p>
          </div>
        </div>
        <dl className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
          <div><dt className="text-gray-500 font-medium">Category</dt><dd className="text-gray-900">{rule.category}</dd></div>
          <div><dt className="text-gray-500 font-medium">Trigger</dt><dd className="text-gray-900">{rule.trigger}</dd></div>
          <div><dt className="text-gray-500 font-medium">Status</dt><dd><span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${rule.status === 'ACTIVE' ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-700'}`}>{rule.status}</span></dd></div>
        </dl>
      </div>
